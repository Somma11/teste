...
function stopCode() {
  ...
  async function verifyUser() {
    GM_xmlhttpRequest({
      method: "GET",
      url: `http://localhost:3000/check-permission?nome=${nickname}&senha=${senha}`,
      onload: function(response) {
        if (response.status === 200) {
          ...
          if (data.status === 'permitido') {
            ...
            console.log('Usuário não permitido ou teste expirado');
            alert(data.status === 'teste expirado' ? 'Seu teste de 2 horas expirou' : 'Você não tem permissão para usar este script.');
